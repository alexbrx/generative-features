...
def main(config):
    ...
    if config.dataset in ["AffectNet"]:
        affectnet_loader = get_loader(
            config.affectnet_image_dir,
            None,
            None,
            config.affectnet_crop_size,
            config.image_size,
            config.batch_size,
            "AffectNet",
            config.mode,
            config.affectnet_emo_descr,
            config.num_workers,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=112,
        help="crop size for the AffectNet dataset",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1,
        help="second (extra) weight for domain classification loss",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="CelebA",
        choices=["CelebA", "RaFD", "AffectNet", "Both"],
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="emotiw",
        choices=["emotiw", "va", "64d_reg", "64d_cls"],
        help="emotiw for categorical emotions, va for valence-arousal",
    )
    parser.add_argument(
        "--use_ccc", type=bool, default=False, help="use ccc loss instead of mse"
    )
    parser.add_argument(
        "--depth_concat",
        type=bool,
        default=True,
        help="perform depthwise concatenation in G",
    )
    parser.add_argument(
        "--d_loss_cls_type",
        type=str,
        default="actv",
        choices=["actv", "pred", "both"],
        help="chosen terms for classification loss",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="PCA visualization number of components kept",
    )
    parser.add_argument(
        "--pca_variant",
        type=str,
        default="quantiles",
        choices=["quantiles", "projections"],
        help="PCA visualization variant",
    )
    # Miscellaneous.
